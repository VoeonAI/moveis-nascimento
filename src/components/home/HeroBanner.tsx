...
const HeroBanner = (
    ...
    const handleWhatsAppClick = () => {
        ...
        const message = encodeURIComponent("Oi, eu estava navegando pelo site e gostaria de ajuda.");
        window.open(`https://wa.me/${normalized}?text=${message}`, "_blank");
    };

    const hasHighlight = finalHighlight.length > 0 && finalTitle.includes(finalHighlight);
    ...
    const renderTitle = () => {
        ...
        return parts.map((part, index) => (<React.Fragment key={index}>
            {part}
            {index < parts.length - 1 && (<span className="text-green-400">{finalHighlight}</span>)}
        </React.Fragment>));
    };
    ...
    return (
        <div className="relative h-[600px] md:h-[700px] overflow-hidden">
            <div
                ...
                <div
                    className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/50 to-black/30"></div>
            </div>
            <div className="relative z-10 max-w-7xl mx-auto px-4 h-full flex items-center">
                <div className="max-w-2xl space-y-6">
                    ...
                    <p className="text-lg md:text-xl text-gray-200">Atendimento personalizado com o Nas e suporte do nosso time de consultores.
                                  </p>
                    <div className="flex flex-col sm:flex-row gap-4 pt-4">
                        <Link to="/catalog">
                            <Button
                                size="lg"
                                className="bg-green-600 hover:bg-green-700 text-white px-8 py-6 text-lg font-semibold rounded-xl shadow-lg transition-all">Ver Catálogo
                        ...
                        <Link to="/sobre">
                            <Button
                                ...
                                <Phone size={20} className="mr-2" />Nossa História
                                              </Button>
                        </Link>
                        <Button
                            size="lg"
                            ...
                            <Phone size={20} className="mr-2" />Falar com o Nas
                                          <ArrowRight size={20} className="ml-2" />
                        </Button>
                    </div>
