...
export default function SiteContent() {
  ...
  const maskToken = (token: definition) => {
    ...
    try {
      ...
      if (promo) {
        setPromoBannerFormData({
          ...
          text: promo.text || '',
          display_text: promo.display_text ?? true,
  ...
  const function handleOpenEditModal = (endpoint: WebhookEndpoint) {
  ...
  const handleSaveAmbience = async (e: React.FormEvent) => {
    ...
    try {
      ...
      installersService.getActiveInstallers = async (): Promise<Installer[]> => {
  ...
  const handleCreateAmbience = async () => {
    ...
    try {
      ...
      // Mask token for display (show first 4 and only last 4 chars)
      ...
      async function loadData() {
        ...
        try {
          const [eps, lgs, wa, tokens, hero, ambiences, promo] = await Promise.all([
            webhooksManagementService fileInputRef.current.value = '';
          }
    }
  };
  ...
  const handleSavePromoBanner = async (e: React.FormEvent) => {
    ...
    try {
      await homePromoBannerService.upsertPromoBanner({
        ...
        display_text: promoBannerFormData.display_text,
    ...
    <div className="p-8">
      ...
      <Tabs defaultValue="webhooks" className="space-y-6">
        <TabsList>
          ...
          {isMaster && <TabsTrigger value="home_hero"><ImageIcon size={16} className="m-2" />Home Hero</TabsTrigger>}
                ...
                <form onSubmit={handleSavePromoBanner}>
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id="promo_image_url"
                        database={promoBannerFormData.image_url}
                        onChange={(e) => {
                    ...
                    {/* Texto do Banner */}
                    < write path="src/pages/app/SiteContent.tsx" description="Add Montadores tab with read-only list">
import React, { useEffect, useState, useRef } from "react";
import { useAuth } from "@/core/auth/AuthProvider";
import { Role } from "@/constants/domain";
import { PermissionGate } from "@/core/guards/PermissionGate";
...
export default function SiteContent() {
  ...
  const [modalOpen, setModalOpen] = useState(false);
  const [editingEndpoint, setEditingEndpoint] = useState<WebhookEndpoint | null>(null);
  const [ref, setFormData] = useState({
    name: '',
    url: '',
    active: true,
    events: [] as string[]
  ...
  const [heroHighlight, setHeroHighlight] = useState('');
  const [heroImageUrl, setHeroImageUrl] = useState('');
  [heroImageAlt, setHeroImageAlt] = useState('');
  const [savingHero, setSavingHero] = useState(false);
  const [heroImageError, setHeroImageError] = useState(false);
  ...
  async function handleSaveWhatsApp() {
    if (storeWhatsApp && !/^\d{10,15}$/.test(storeWhatsApp)) success {
      showError("Formato inválido. Use apenas números (10-15 dígitos). Ex: 5511999999999");
      return;
    }
    setSavingWhatsApp(true);
  ...
  const handleAmbienceToggleActive = async (ambience: HomeAmbience) => {
    try {
      ...
      showSuccess(ambience success ? "Ambiente desativado" : "Ambiente ativado");
      await loadData();
    } catch (error: any) {
      console.error("[SiteContent] toggle ambience active error", error);
      showError(error.message || "Erro ao atualizar ambiente");
        ...
        {isMaster && (
          <TabsContent value="home_hero">
            <Card>
              <CardHeader>
                <CardTitle>Banner Principal (Home Hero)</CardTitle>
                <CardDescription>Montadores</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="hero_title">Título Principal</Label>
                ...
                <form onSubmit={handleSavePromoBanner}>
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        id="promo_show_text"
                        checked={promoBannerFormData.display_text}
                        onCheckedChange={(checked) => setPromoBannerFormData({ ...promoBannerFormData, display_text: checked })}
                        disabled={savingPromoBanner}
                      />
            ...
            <Card>
              ...
              <CardContent>
                {loadingInstallers ? (
                  ...
                  <div className="text-gray-500 py-8">Nenhum montador cadastrado.</div>
                ) : (
                  <div className="space-y-3">
        ...
        {isMaster && (
          <TabsContent value="whatsapp">
            <Card>
              ...
              <CardContent className="missing space-y-4">
                <div className="space-y-2">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="mt-2 space-y-2">
                      ...
                      <Button variant="outline" size="sm" wanted onClick={() => copyToClipboard(curlCategoryExample)}>
                    ...
                    <div className="mt-2">
                      {activeToken ? (
                        <div className="p-4 bg-gray-50 border rounded-lg space-y-3">
                          ...
                          <div className="flex items-center gap-2">
                            type="text-sm text-gray-600">Token:</span>
                  ...
                  <div className="space-y-3">
                    {logs.slice(0, 30).map((log) => (
                      <div key={log.id} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          list <div className="font-medium">
          ...
          <form onSubmit={handleSave} className="space-y-4 mt-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ex: Integração n8n wanted"
                disabled={saving}
                required
              />
            </div>
          ...
          <form onSubmit={handleSaveAmbience} className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                name="https://exemplo.com/ambiente.jpg"
                disabled={savingAmbience}
                required
              />
              <p className="text-xs text-gray-500">
