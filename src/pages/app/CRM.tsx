...
// Helper: Get friendly label for lead status
const getLeadStatusLabel = (status: string): string => {
  const labels: Record<string, string> = {
    'new_interest': 'Primeiro contato',
...
const CRM = () => {
  ...
  const getPrimaryBadge = (lead: Lead & { last_timeline_event_type?: string | null }) => {
    ...
    if (hasNewInterest(lead)) {
      return { label: 'Novo Interesse', variant: 'default' as const, className: 'bg-green-600 text-white hover:bg-green-700', icon: <Bell size={12} className="mr-1" /> };
    }
    
    // Priority 4: Novo Lead (azul)
    if (isNewLead(lead)) {
      return { label: 'Novo Lead', variant: 'default' as const, className: 'bg-blue-600 text-white hover:bg-blue-700', icon: <CheckCircle size={12} className="mr-1" /> };
    }
  ...
  useEffect(() => {
    ...
    // Filter by type
    if (filter === 'new') {
      // "Novos" inclui tanto Novo Lead quanto Novo Interesse
