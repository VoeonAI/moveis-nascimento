...
export const crmService = {
  ...
  // PATCH: Adicionar join com products E incluir lead_id
  ...
  async setFollowUp(leadId: string, needed: boolean, at?: Date | string): Promise<void> {
    ...
    const { error } = await supabase
      .from('leads')
      .update({
<dyad-write path="src/services/crmService.ts" description="Fixed crmService to include lead_id in opportunity queries and make webhooks best-effort">
import { supabase } from '@/core/supabaseClient';
import { Lead, Opportunity } from '@/types';
import { OpportunityStage } from '@/constants/domain';
import { webhooksService } from './webhooksService';
