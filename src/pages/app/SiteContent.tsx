...
export default function SiteContent() {
  ...
  // Installer Creation State
  const [installerCreateModalOpen, setInstallerCreateModalOpen] = useState(false);
  ...
  const envelopeConfig = {
    ...
    source: { app: "moveis-nascimento-site", env: "production", channel: "site" },
    data: {},
    meta: {},
  };
  ...
  // Installer Creation Handlers
  const handleOpenInstallerCreateModal = () => {
    setInstallerFormData({
      name: '',
      phone: '',
      city: '',
    });
    setInstallerCreateModalOpen(true);
  };

  const handleSaveInstaller = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!installerFormData.name.trim()) {
      showError("Nome é obrigatório");
      return;
    }

    if (!installerFormData.phone.trim()) {
    ...
    try {
      await installersService.createInstaller({
        name: installerFormData.name,
        phone: installerFormData.phone,
        city: installerFormData.city || null,
      });
      showSuccess("Montador cadastrado com sucesso!");
      setInstallerCreateModalOpen(false);
      setInstallerFormData({
        name: '',
        phone: '',
        city: '',
      ...
      console.error("[SiteContent] save installer error", error);
  ...
  return (
    <div className="p-8">
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button onClick={handleOpenInstallerCreateModal}>
                    <Plus size={16} className="mr-2" />
                    Novo Montador
                  </Button>
                </div>
              ...
              <CardContent>
                {loadingInstallers ? (
                  ...
                  <div className="text-gray-500 py-8 text-center">
                    Nenhum montador cadastrado. Clique em "Novo Montador" para adicionar o primeiro.
                  </div>
                ) : (
                  <div className="space-y-3">
      ...
      {/* Installer Create Modal */}
      <Dialog open={installerCreateModalOpen} onOpenChange={setInstallerCreateModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <form onSubmit={handleSaveInstaller} className="space-y-4 mt-4">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="installer_phone"
                value={installerFormData.phone}
                onChange={(e) => setInstallerFormData({ ...installerFormData, phone: e.target.value })}
            ...
            <div className="space-y-2">
              ...
              <Input
                id="installer_city"
                value={installerFormData.city}
                onChange={(e) => setInstallerFormData({ ...installerFormData, city: e.target.value })}
            ...
            <DialogFooter className="pt-4">
              ...
              <Button type="submit" disabled={savingInstaller}>
                {savingInstaller ? (
                  <>
                    <Loader2 size={16} className="mr-2 animate-spin" />
                    Salvando...
