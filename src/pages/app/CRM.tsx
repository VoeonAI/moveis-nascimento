...
const CRM = () => {
  ...
  const isFollowUpOverdue = (lead: Lead) => {
    if (!lead.follow_up_needed || !lead.follow_up_at) return false;
    const today = startOfDay(new Date());
    const followUpDay = startOfDay(new Date(lead.follow_up_at));
    return isBefore(followUpDay, today);
  };

  // Helper: Check if follow-up is today
  const isFollowUpToday = (lead: Lead) => {
    if (!lead.follow_up_needed || !lead.follow_up_at) return false;
    const today = startOfDay(new Date());
    const followUpDay = startOfDay(new Date(lead.follow_up_at));
    return isSameDay(followUpDay, today);
  };

  // Helper: Get primary badge for a lead based on priority
  const getPrimaryBadge = (lead: Lead & { last_timeline_event_type?: string | null }) => {
    // Priority 1: Atrasado
    if (isFollowUpOverdue(lead)) {
      return { label: 'Atrasado', variant: 'destructive' as const, icon: <AlertCircle size={12} className="mr-1" /> };
    }
    
    // Priority 2: Follow-up Hoje
    if (isFollowUpToday(lead)) {
      return { label: 'Follow-up Hoje', variant: 'default' as const, className: 'bg-orange-600 text-white hover:bg-orange-700', icon: <CalendarIcon size={12} className="mr-1" /> };
    }
    
    // Priority 3: Novo Interesse
    if (hasNewInterest(lead)) {
      return { label: 'Novo Interesse', variant: 'default' as const, className: 'bg-blue-600 text-white hover:bg-blue-700', icon: <Bell size={12} className="mr-1" /> };
    }
    
    // Priority 4: Novo
    if (isNewLead(lead)) {
      return { label: 'Novo', variant: 'default' as const, className: 'bg-green-600 text-white hover:bg-green-700', icon: <CheckCircle size={12} className="mr-1" /> };
    }
          ...
          {filteredLeads.length === 0 && !error ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredLeads.map((lead) => {
                const primaryBadge = getPrimaryBadge(lead);
                    ...
                    <CardHeader>
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex gap-1">
                          {primaryBadge && (
                            <Badge className={primaryBadge.className} variant={primaryBadge.variant}>
                              {primaryBadge.icon}
                              {primaryBadge.label}
                            </Badge>
                          )}
            ...
            {leadDetails && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  {/* Show Hard Delete option for# MELHORIA DE UX NO CRM — DESTACAR FOLLOW-UPS DO DIA E ATRASADOS NOS CARDS

---

## 1. Regra de prioridade dos badges

1. **Atrasado** (prioridade máxima) - Follow-up com data no passado
2. **Follow-up Hoje** - Follow-up com data igual a hoje
3. **Novo Interesse** - Último evento da timeline é `opportunity_created`
4. **Novo** - Lead recém-criado (últimos 3 dias)

---

## 2. Arquivo alterado

`src/pages/app/CRM.tsx`

---

## 3. Patch mínimo aplicado

<dyad-write path="src/pages/app/CRM.tsx" description="Added priority-based badge highlighting for urgent follow-ups (atrasado/follow-up hoje) in CRM cards">
import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { crmService, Opportunity, TimelineEvent } from '@/services/crmService';
import { ordersService } from '@/services/ordersService';
